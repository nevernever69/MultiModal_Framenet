...
This script loads the Gemma model pipeline and processes a set of images using
a dataset approach for better GPU utilization, extracting structured details
...
def collect_image_paths():
    """
    Collects valid image paths and creates a dataset.
    
    Returns:
        Dataset: A HuggingFace dataset of valid image paths.
    """
    ...
    for path in image_paths:
        if os.path.exists(path):
            valid_paths.append({"image_path": path})
        else:
            print(f"Image path {path} does not exist. Skipping.")
    
    return Dataset.from_list(valid_paths)
...
def process_batch(batch, pipe):
    ...
    Args:
        batch (dict): Dictionary containing image paths.
        pipe: The loaded Transformers pipeline.
        
    Returns:
        dict: Dictionary with processing results.
    """
    # Create prompts for each image path
    prompts = [create_prompt(path) for path in batch["image_path"]]
    
    # Process the batch of prompts
    start_time = time.time()
    results = pipe(text=prompts, batch_size=2, max_new_tokens=1200)
    ...
    for i, result in enumerate(results):
        ...
        try:
            if "generated_text" in result[0] and len(result[0]["generated_text"]) > 0:
                ...
                processed_results.append({
                    ...
                    "processing_time": duration / len(batch["image_path"])
                })
                print(f"Processed {image_path} in {duration / len(batch['image_path']):.2f} seconds per image")
            else:
                print(f"Warning: No valid response for {image_path}. Skipping.")
                processed_results.append({
                    "image_path": image_path,
...
def main():
    ...
    dataset = collect_image_paths()
    
    if len(dataset) == 0:
        print("No valid images found. Exiting.")
        return
    
    print(f"Processing {len(dataset)} images using dataset approach...")
    ...
    for i in range(0, len(dataset), batch_size):
        ...
        results = process_batch(batch_dict, pipe)
