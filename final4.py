...
class StatsTracker:
    ...
    def print_final_report(self):
        ...
        print(f"  Flash attention: {'Enabled' if stats['model_config']['flash_attention'] else 'Disabled'}")
        print("="*80)
...
def create_optimized_pipeline():
    ...
    # Load processor and model separately for more control
    print(f"Loading Gemma model and processor...")
    start_time = time.time()
    
    processor = AutoProcessor.from_pretrained("unsloth/gemma-3-12b-it-bnb-4bit")
...
def main(image_paths, output_csv, batch_size=BATCH_SIZE):
    ...
    print(f"Using flash attention: {USE_FLASH_ATTENTION}")
    print(f"{'='*50}\n")
    ...
    with open(output_csv, 'w', newline='', encoding='utf-8') as file:
        ...
        try:
            for i, batch in enumerate(dataloader):
                ...
                print(f"Batch {i+1}/{len(dataloader)} complete. {stats_tracker.get_progress_string()}")
                
                # Periodically save stats
                if (i+1) % 5 == 0 or (i+1) == len(dataloader):
                    stats_file = f"{os.path.splitext(output_csv)[0]}_stats.json"
