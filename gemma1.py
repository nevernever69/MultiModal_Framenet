...
# Custom dataset for loading news images
...
# Custom collate function that ensures images are tensors
def custom_collate_fn(batch):
    """
    Collate function for DataLoader.
    Converts any PIL images in the batch to tensors and collates image paths.
    """
    images, image_paths = zip(*batch)
    tensor_images = []
    for img in images:
        # If img is not a tensor, convert it using default_transform.
        if not isinstance(img, torch.Tensor):
            tensor_img = default_transform(img)
        else:
            tensor_img = img
    ...
    try:
        ...
        # If images have varying sizes, fall back to a list
        images_tensor = tensor_images
    return images_tensor, list(image_paths)

# Create dataset and dataloader for batch processing.
dataset = NewsImageDataset('images', transform=default_transform)
dataloader = DataLoader(dataset, batch_size=4, shuffle=False, num_workers=4, collate_fn=custom_collate_fn)

# Your analysis prompt text
PROMPT_TEXT = """
...
def analyze_images_batch(batch, pipe):
    ...
    messages_batch = []
    image_paths = []
    for image, image_path in zip(*batch):
        # Wrap the image and prompt in the expected format.
        messages = [{
            "role": "user",
            "content": [
    ...
    try:
        # Process the batch; ensure the pipeline supports batched inputs.
...
def main():
    ...
    with open(csv_file, mode='w', newline='', encoding='utf-8') as file:
        ...
        for batch in dataloader:
            # The custom collate function returns (images_tensor, image_paths)
